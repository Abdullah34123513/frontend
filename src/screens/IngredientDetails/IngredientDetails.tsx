...
export const IngredientDetails = (): JSX.Element => {
  ...
  const ingredientData = {
    ...
    storage: "Fridge",
    description: "A fresh and flavorful sauce made with tomatoes, onions, peppers, and herbs. Perfect for adding zest to your dishes.",
    nutritionPer100g: {
            ...
            <Card className="sticky top-24">
              <CardContent className="p-6">
                ...
                <div className="space-y-3 mb-6">
                  {Object.entries(ingredientData.nutritionPer100g).map(([key, data]) => (
                    <div key={key} className="flex items-center justify-between py-2 border-b border-gray-100">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className={`px-2 py-1 rounded text-xs font-medium ${getNutritionBgColor(data.percentage)} ${getNutritionColor(data.percentage)}`}>
                          {data.percentage}%
                        </div>
                      </div>
                    </div>
                ...
                <div className="mb-6 p-4 bg-green-50 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-gray-700">Glycemic Index</span>
                ...
                <div className="space-y-3">
                  <Button className="w-full bg-[#22ae4b] hover:bg-[#1c9a40] text-white">
                    ...
                    {language === 'ar' ? 'أضف إلى قائمة التسوق' : 'Add to shopping list'}
                  </Button>
          ...
          <div className="lg:col-span-8">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">
                    {language === 'ar' ? `في ${ingredientData.recipeCount.toLocaleString()} وصفة` : `In ${ingredientData.recipeCount.toLocaleString()} Recipes`}
                  </h2>
                  <p className="text-gray-600">
                    {language === 'ar' ? 'وصفات تستخدم هذا المكون' : 'Recipes that use this ingredient'}
                  </p>
                </div>
                  ...
                  >
                    {sortOptions.map((option) => (
                      <option key={option.value} value={option.value}>
                        {language === 'ar' ? 'ترتيب حسب: ' : 'Sort by: '}{option.label}
                      </option>
                    ))}
                  </select>
            ...
            <div className="text-center mt-8">
              <Button variant="outline" className="px-8">
                {language === 'ar' ? 'تحميل المزيد من الوصفات' : 'Load More Recipes'}
              </Button>
            </div>
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">
                        {ingredientData.name} - {language === 'ar' ? 'جميع العناصر الغذائية' : 'All Nutrients'}
                      </h3>
              ...
              <div className="p-6 max-h-[calc(90vh-120px)] overflow-y-auto">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {ingredientData.allNutrients.vitamins.map((nutrient, index) => (
                        <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {nutrient.percentage !== null && (
                              <div className={`px-2 py-1 rounded text-xs font-medium ${getNutritionBgColor(nutrient.percentage)} ${getNutritionColor(nutrient.percentage)}`}>
                                {nutrient.percentage}%
                              </div>
                            )}
                          </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {ingredientData.allNutrients.minerals.map((nutrient, index) => (
                        <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100">
                          <span className="text-sm font-medium text-gray-700">{nutrient.name}</span>
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-semibold text-gray-900">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                      ...
                      {language === 'ar' ? 'عناصر أخرى' : 'Other Nutrients'}
                    </h4>
                    <div className="space-y-3">
                      {ingredientData.allNutrients.otherNutrients.map((nutrient, index) => (
                        <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100">
                          <span className="text-sm font-medium text-gray-700">{nutrient.name}</span>
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-semibold text-gray-900">
                ...
                <div className="mt-8 p-4 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-xs text-gray-500 text-center mt-2">
                    {language === 'ar' ? 'المصدر: قاعدة بيانات التغذية الأمريكية' : 'Source: USDA National Nutrient Database'}
                  </p>
                </div>
              </div>
            </CardContent>
