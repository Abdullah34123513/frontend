...
export const GoalsManagement = (): JSX.Element => {
  ...
  const [userGoals, setUserGoals] = useState<Goal[]>([
    {
      ...
      milestones: [
        { id: 1, value: 82, description: 'First 3kg lost', achieved: true, achievedDate: '2024-01-15' },
    ...
    },
    {
      ...
      milestones: [
        { id: 1, value: 4, description: 'Consistent 4 workouts/week', achieved: true, achievedDate: '2024-01-20' },
    ...
    },
    {
      ...
      milestones: [
        { id: 1, value: 2, description: '2 healthy meals daily', achieved: true, achievedDate: '2024-01-18' },
      ...
      if (goal.id === goalId) {
        ...
        if (goal.startValue !== undefined && goal.targetValue !== undefined) {
          if (goal.category === 'weight' && goal.id === 'lose-weight') {
            ...
            updatedGoal.progress = Math.max(0, Math.min(100, 
              ((goal.startValue - value) / (goal.startValue - goal.targetValue)) * 100
            ));
          } else {
            // For other goals: progress = (current - start) / (target - start) * 100
            updatedGoal.progress = Math.max(0, Math.min(100, 
              ((value - goal.startValue) / (goal.targetValue - goal.startValue)) * 100
            ));
          }
        }
  ...
  const removeGoal = (goalId: string) => {
    if (window.confirm(language === 'ar' ? 'هل أنت متأكد من حذف هذا الهدف؟' : 'Are you sure you want to remove this goal?')) {
  ...
  const addNewGoal = (goalTemplate: any) => {
    const newGoal: Goal = {
      ...
      currentValue: 0,
      targetValue: 0,
      unit: goalTemplate.category === 'weight' ? 'kg' : 
            goalTemplate.category === 'fitness' ? 'workouts/week' :
            goalTemplate.category === 'health' ? 'meals/day' : 'hours/week',
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      ...
      case 'declining': return <Decline className="w-4 h-4 text-red-500" />;
  ...
  const achievedMilestones = userGoals.reduce((sum, goal) => 
          ...
          <CardContent className="p-4 sm:p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center ${selectedGoal.bgColor}`}>
                  <selectedGoal.icon className={`w-4 h-4 sm:w-5 sm:h-5 ${selectedGoal.color}`} />
                </div>
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">
                    {language === 'ar' ? selectedGoal.titleAr : selectedGoal.title}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center">
                  <div>
                    <div className="text-xs sm:text-sm text-gray-600">{language === 'ar' ? 'البداية' : 'Start'}</div>
                    <div className="font-bold text-sm sm:text-lg">{selectedGoal.startValue}<span className="text-xs">{selectedGoal.unit}</span></div>
                  </div>
                  <div>
                    <div className="text-xs sm:text-sm text-gray-600">{language === 'ar' ? 'الحالي' : 'Current'}</div>
                    <div className="font-bold text-sm sm:text-lg text-[#22ae4b]">{selectedGoal.currentValue}<span className="text-xs">{selectedGoal.unit}</span></div>
                  </div>
                  <div>
                    <div className="text-xs sm:text-sm text-gray-600">{language === 'ar' ? 'الهدف' : 'Target'}</div>
                    <div className="font-bold text-sm sm:text-lg">{selectedGoal.targetValue}<span className="text-xs">{selectedGoal.unit}</span></div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">
                  {language === 'ar' ? `القيمة الجديدة (${selectedGoal.unit})` : `New Value (${selectedGoal.unit})`}
                </label>
                <Input
                  type="number"
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={language === 'ar' ? 'أضف ملاحظة حول تقدمك...' : 'Add a note about your progress...'}
                  className={`h-10 sm:h-12 border-gray-300 focus:border-[#22ae4b] focus:ring-[#22ae4b] text-sm ${isRTL ? 'text-right' : 'text-left'}`}
                />
              </div>
              ...
              {newProgressValue && !isNaN(parseFloat(newProgressValue)) && (
                <div className="bg-blue-50 rounded-lg p-3 sm:p-4">
                  <h4 className="font-medium text-sm sm:text-base text-blue-900 mb-2">
                    {language === 'ar' ? 'معاينة التحديث' : 'Update Preview'}
                  </h4>
                  <div className="text-xs sm:text-sm text-blue-700">
                    {selectedGoal.id === 'lose-weight' 
                      ? `${language === 'ar' ? 'تغيير الوزن:' : 'Weight change:'} ${(parseFloat(newProgressValue) - (selectedGoal.currentValue || 0)).toFixed(1)}${selectedGoal.unit}`
                      : `${language === 'ar' ? 'التغيير:' : 'Change:'} ${(parseFloat(newProgressValue) - (selectedGoal.currentValue || 0)).toFixed(1)}${selectedGoal.unit}`
                    }
                  </div>
                </div>
              )}
  ...
  const EditGoalModal = () => {
    ...
    const handleSave = () => {
      const updatedGoal = {
        ...
        descriptionAr: language === 'ar' ? editData.description : editingGoal.descriptionAr
      };
      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-2 sm:p-4">
        <Card className="w-full max-w-sm sm:max-w-lg bg-white shadow-2xl">
          <CardContent className="p-4 sm:p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center ${editingGoal.bgColor}`}>
                  <editingGoal.icon className={`w-4 h-4 sm:w-5 sm:h-5 ${editingGoal.color}`} />
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-gray-900">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">
                    {language === 'ar' ? editingGoal.titleAr : editingGoal.title}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditData(prev => ({ ...prev, targetValue: e.target.value }))}
                  className="h-10 sm:h-12 border-gray-300 focus:border-[#22ae4b] focus:ring-[#22ae4b] text-sm"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className={`w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:border-[#22ae4b] focus:ring-[#22ae4b] resize-none text-sm ${isRTL ? 'text-right' : 'text-left'}`}
                  placeholder={language === 'ar' ? 'اوصف هدفك...' : 'Describe your goal...'}
                />
              </div>
                  ...
                  >
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex items-start gap-3 sm:gap-4">
                        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center bg-white`}>
                          <goal.icon className={`w-5 h-5 sm:w-6 sm:h-6 ${goal.color}`} />
                        </div>
              ...
              {filteredAvailableGoals.length === 0 && (
                <div className="text-center py-6 sm:py-8">
                  ...
                  <p className="text-sm sm:text-base text-gray-500">
                    {language === 'ar' ? 'لقد أضفت جميع الأهداف المتاحة' : "You've added all available goals"}
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm text-[#22ae4b] font-medium">{language === 'ar' ? 'الأهداف' : 'Goals'}</div>
                <h1 className="text-3xl font-bold text-gray-900">{language === 'ar' ? 'إدارة الأهداف' : 'Goals Management'}</h1>
              </div>
            </div>
          </div>
          <p className="text-gray-600">{language === 'ar' ? 'تتبع وإدارة أهدافك الصحية والغذائية' : 'Track and manage your health and nutrition goals'}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <div className="text-sm text-gray-600">{language === 'ar' ? 'أهداف نشطة' : 'Active Goals'}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <div className="text-sm text-gray-600">{language === 'ar' ? 'متوسط التقدم' : 'Average Progress'}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-gray-900">{achievedMilestones}</div>
              <div className="text-sm text-gray-600">{language === 'ar' ? 'إنجازات محققة' : 'Milestones Achieved'}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <div className="text-sm text-gray-600">{language === 'ar' ? 'أهداف مكتملة' : 'Completed Goals'}</div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">{language === 'ar' ? 'أهدافي' : 'My Goals'}</h2>
              <Button 
                    ...
                    <CardContent className="p-6">
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 text-sm">
                              <div className={`flex items-center gap-1 px-2 py-1 rounded ${getTrendColor(trend)}`}>
                                {getTrendIcon(trend)}
                              ...
                              <div className="flex items-center gap-1 text-gray-600">
                                ...
                                <span>
                                  {daysRemaining > 0 
                                    ...
                                    : `${Math.abs(daysRemaining)} ${language === 'ar' ? 'يوم متأخر' : 'days overdue'}`
                                  }
                                </span>
                              </div>
                            </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        <div className="bg-gray-50 rounded-lg p-4 text-center">
                          <div className="text-sm text-gray-600 mb-1">{language === 'ar' ? 'البداية' : 'Start'}</div>
                          <div className="text-2xl font-bold text-gray-900">{goal.startValue}{goal.unit}</div>
                        ...
                        <div className="bg-[#22ae4b] text-white rounded-lg p-4 text-center">
                          <div className="text-sm text-white/90 mb-1">{language === 'ar' ? 'الحالي' : 'Current'}</div>
                          <div className="text-2xl font-bold">{goal.currentValue}{goal.unit}</div>
                          <div className="text-xs text-white/80">
                            {goal.progressHistory.length > 0 
                              ? new Date(goal.progressHistory[goal.progressHistory.length - 1].date).toLocaleDateString()
                              : language === 'ar' ? 'لم يتم التحديث' : 'Not updated'
                            }
                          </div>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-4 text-center">
                          <div className="text-sm text-gray-600 mb-1">{language === 'ar' ? 'الهدف' : 'Target'}</div>
                          <div className="text-2xl font-bold text-gray-900">{goal.targetValue}{goal.unit}</div>
                          <div className="text-xs text-gray-500">{new Date(goal.targetDate).toLocaleDateString()}</div>
                        </div>
                      </div>
                      ...
                      <div className="mb-6">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-sm font-bold text-[#22ae4b]">{goal.progress.toFixed(1)}%</span>
                        </div>
                              ...
                              >
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
                                  ...
                                  {milestone.achieved ? (
                                    ...
                                    <span className="text-white text-xs font-bold">{milestone.id}</span>
                                  )}
                                </div>
                                <div className="flex-1">
                                  ...
                                  {milestone.achieved && milestone.achievedDate && (
                                    <div className="text-xs text-green-600">
                                      {language === 'ar' ? 'تم في' : 'Achieved on'} {new Date(milestone.achievedDate).toLocaleDateString()}
                                    </div>
                                  )}
                                </div>
                              </div>
                      ...
                      <div>
                        ...
                        >
                          <Plus className="w-4 h-4 mr-2" />
                          {language === 'ar' ? 'إضافة تقدم جديد' : 'Add New Progress'}
                        </Button>
                      </div>
                    </CardContent>
                  </Card>
                ...
                <Card className="text-center py-12">
                  <CardContent>
                    ...
                    <p className="text-gray-500 mb-6">
                      {language === 'ar' ? 'ابدأ بإضافة هدفك الأول' : 'Start by adding your first goal'}
                    </p>
                    <Button 
                      onClick={() => setShowAddGoalModal(true)}
                      className="bg-[#22ae4b] hover:bg-[#1c9a40] text-white"
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-24 space-y-6">
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">{language === 'ar' ? 'الأهداف النشطة' : 'Active Goals'}</span>
                      <span className="font-semibold text-[#22ae4b]">{activeGoals}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">{language === 'ar' ? 'متوسط التقدم' : 'Average Progress'}</span>
                      <span className="font-semibold text-gray-900">{averageProgress}%</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">{language === 'ar' ? 'المعالم المحققة' : 'Milestones Achieved'}</span>
                      <span className="font-semibold text-purple-600">{achievedMilestones}/{totalMilestones}</span>
                    </div>
                    <Separator />
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">{language === 'ar' ? 'الأهداف المكتملة' : 'Completed Goals'}</span>
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">
                    {language === 'ar' ? 'نصائح الأهداف' : 'Goal Tips'}
                  </h3>
                  <div className="space-y-3 text-sm text-gray-600">
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-[#22ae4b] rounded-full mt-2 flex-shrink-0" />
                      <p>{language === 'ar' ? 'حدث تقدمك بانتظام للحصول على أفضل النتائج' : 'Update your progress regularly for best results'}</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-[#22ae4b] rounded-full mt-2 flex-shrink-0" />
                      <p>{language === 'ar' ? 'ضع أهدافاً واقعية وقابلة للتحقيق' : 'Set realistic and achievable goals'}</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-[#22ae4b] rounded-full mt-2 flex-shrink-0" />
                      <p>{language === 'ar' ? 'احتفل بالمعالم الصغيرة في طريقك' : 'Celebrate small milestones along the way'}</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-[#22ae4b] rounded-full mt-2 flex-shrink-0" />
                      <p>{language === 'ar' ? 'استخدم الملاحظات لتتبع ما يعمل' : 'Use notes to track what works'}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
